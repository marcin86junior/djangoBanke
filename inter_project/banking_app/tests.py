...
class UserViewSetTest(TestCase):
    ...
    def test_internal_transaction(self):
        ...
        response = self.client.post(
            "/api/transaction/", transaction_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_external_transaction(self):
        ...
        self.assertEqual(
            updated_balance_from_after_transfer,
            Decimal(expected_balance_from_after_transfer),
        )

        updated_balance_to_after_transfer = Account.objects.get(id=2).balance
        expected_balance_to_after_transfer = 50
        self.assertEqual(
            updated_balance_to_after_transfer,
            Decimal(expected_balance_to_after_transfer),
        )
